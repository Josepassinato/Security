...
- [~] T2.2 LangGraph parallel topology (P0, M) → T2.1 — partial / wip — `RouterOrchestrator` (`services/agents/app/orchestrator/router.py`) exposes a `stream(state, *, topology=...)` async generator that emits the same `step` / `done` event surface as `InvestigatorOrchestrator.stream`, writes best-effort ledger events + the final Markdown / HTML report as artifacts, and short-circuits the auto-close path. Deterministic report synthesis lives in `app/orchestrator/report.py`. **`/investigate` swap landed behind a flag**: the new `stream_kwargs(case_id, alert_summary, raw_alert, tenant_id, run_id=None)` adapter on `RouterOrchestrator` coerces string IDs to UUIDs (`uuid5` against a fixed namespace, deterministic across processes), enriches `step` events with `case_id` / `run_id` / `kind` / `node` / `ts` to match the investigator surface, and synthesises explicit `error` events from `state.status == FAILED` `done` events. `services/agents/app/api/investigate.py` reads `AISOC_INVESTIGATE_USE_ROUTER` at call time (default off; flipping mid-process takes effect on the next call without a restart) and dispatches both the HTTP POST and WebSocket paths through `_investigate_stream(...)`. Coverage: `test_orchestrator_router_stream.py` (8) + `test_router_report.py` (3) + `test_orchestrator_router_stream_kwargs.py` (17 — event-shape parity, error synthesis, ID coercion, `alert_summary` landing on inner state) + `test_investigate_router_flag.py` (27 — flag parsing, dispatcher selection, call-time evaluation, run_id pass-through, module-singleton stability). All 55 router-adjacent tests green. Remaining: retire the legacy `InvestigatorOrchestrator` once the flag has been on in production long enough to verify parity.
