...
- [x] T2.3 LLM-input contract (P0, M) → T2.1 — `87c151a7` (`app/llm/contract.py` + `safe_ainvoke` / `safe_astream` + `make_safe_chat_model`; `test_llm_contract.py` asserts raw OCSF JSON is rejected and `summarize_structure_for_llm` output passes). **Wired** on contextual chat (`api/contextual.py`), auto-triage, and investigator subgraph agents — full survey of `services/agents/app/` (T2.3 branch) confirmed every direct chat-model call already routes through `safe_ainvoke` / `safe_astream`; remaining `.ainvoke` / `.astream` call sites are LangGraph control-flow on `_graph` / `investigation_graph`. Added (a) streaming + wrapper + secret-pattern + enforcement-toggle coverage in `tests/test_llm_contract_extra.py` (10 cases) and (b) an **AST-based static regression gate** in `tests/test_llm_contract_no_bypass.py` that scans every file under `services/agents/app/` for forbidden direct `.ainvoke` / `.astream` calls with an explicit receiver allowlist (`_graph`, `investigation_graph`, `graph`) and file allowlist (`app/llm/contract.py` itself). The gate ships with self-tests (synthetic bypass trips it, allowlisted receiver doesn't), so any future regression — including a new agent module — fails CI before it can leak raw logs to an LLM.
