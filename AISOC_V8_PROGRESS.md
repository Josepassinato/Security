...
- [x] T2.2 LangGraph parallel topology (P0, M) → T2.1 — `RouterOrchestrator` (`services/agents/app/orchestrator/router.py`) shipped with `classify_signals` → fan-out to `phishing` / `identity` / `cloud` / `insider_threat` via `asyncio.gather` + `Join` node that dedupes MITRE / findings. Behind `AISOC_AGENT_PARALLEL_TOPOLOGY` flag (`PARALLEL_TOPOLOGY_FLAG`). Eval gate: 17 unit tests (`tests/test_orchestrator_parallel.py`) including ≥30% wall-clock win over sequential under the substrate harness and auto-close short-circuit. **Wired** into HTTP via new `POST /api/v1/cases/{case_id}/triage` + `GET /api/v1/triage/{run_id}` (`services/agents/app/api/triage.py`); request body accepts a per-run `topology` override, response carries the resolved topology + signals + wall-clock telemetry. 8 integration tests (`tests/test_triage_endpoint.py`) cover env-flag flip, body override, end-to-end fan-out, auto-close skip, 404 on unknown run, and minimal-body coercion. The legacy `/investigate` linear path on `InvestigatorOrchestrator` is **unchanged** — no blast radius on existing demos.
