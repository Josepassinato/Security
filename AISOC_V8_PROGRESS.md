...
- [~] T2.2 LangGraph parallel topology (P0, M) → T2.1 — partial / wip — `RouterOrchestrator` (`services/agents/app/orchestrator/router.py`) now exposes a `stream(state, *, topology=...)` async generator that emits the same `step` / `done` event surface as `InvestigatorOrchestrator.stream`, writes best-effort ledger events + the final Markdown / HTML report as artifacts, and short-circuits the auto-close path. Deterministic report synthesis lives in the new `app/orchestrator/report.py`. Coverage: `test_orchestrator_router_stream.py` (8 tests — event ordering, monotonic `seq`, parallel vs sequential, auto-close, ledger ordering, ledger-down graceful degradation) + `test_router_report.py`. Remaining: swap `/investigate` over to the router and retire the legacy `InvestigatorOrchestrator` once parity is operator-verified.
