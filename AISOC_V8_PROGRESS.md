...
- [x] T3.4 /hunt NL surface (P0, S-M) — endpoints/UI/redirect shipped; scheduler now executes real ES|QL via the shared `services/api/app/services/esql_runner.py` module (SSRF guard + air-gap policy + LIMIT enforcement reused by `/nl-query/execute` and the background worker; tests in `tests/test_esql_runner.py` + `tests/test_hunt_scheduler_execute.py`)
...
- 2026-05-15 — T3.4 P0 completed (scheduler now executes real ES|QL): replaced the `_execute_hunt` stub with a shared `services/api/app/services/esql_runner.py` module that both the `/nl-query/execute` endpoint and the saved-hunt scheduler now consume. One code path enforces the SSRF guard (host validated against `ES_URL`/`ELASTICSEARCH_URL`/`OPENSEARCH_URL`, URL reconstructed from validated scheme+netloc only — no user-supplied path/query reaches the network), the air-gap policy (`enforce_airgap_for_url` runs unconditionally so `AISOC_AIRGAPPED=true` blocks egress), and the `LIMIT` cap (`HUNT_SCHEDULER_MAX_ROWS`, default 500). Scheduler reads the stored `translated_query["esql"]` (no re-translation in the background — that's the endpoint's job), returns `len(rows)` so `run_once` can decide whether to fire `_open_case_for_hits`, and quietly returns `0` when ES is not configured (dev installs without ES wired up stay silent instead of spamming `exception`). Errors (`AirgapViolation`, `ValueError`, `ESQLExecutionError`) propagate to `run_once` which skips the `last_run_at` bump so the hunt retries on the next sweep. `nl_query.py` refactored at the same time to import the shared module (its private `_validate_es_url` was deleted), keeping the two execution surfaces from drifting on the security guards. Tests: 22 cases in `services/api/tests/test_esql_runner.py` (URL validation, credential resolution, air-gap propagation, LIMIT enforcement, error wrapping) + 9 cases in `services/api/tests/test_hunt_scheduler_execute.py` (all four skip paths, happy path with row count, three error-propagation paths). The existing saved-hunts endpoint + tenant-isolation suites (47 tests) continue to pass unmodified.
