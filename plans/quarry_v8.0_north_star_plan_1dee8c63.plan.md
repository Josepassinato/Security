# Quarry v8.0 — North-star Plan

> **Status**: Locked plan — implement as specified, do not edit.
> **Captured**: 2026-05-15
> **Hash**: `1dee8c63`
> **Tracking**: `QUARRY_V8_PROGRESS.md` mirrors progress; this file is the source of truth.
...
9. **Hosted multi-tenant SaaS at app.quarry.dev** (early-access waitlist) running the same code as self-hosted. Self-hosted stays first-class.
...
**Wiring:**
- Kafka consumer group `quarry-graph-writer` reading `security.events`
...
**Acceptance:** Doc renders on docs.quarry.dev/architecture/graph-schema with full ER diagram. `scripts/export_graph_schema.py --check` fails CI if schema drifts without doc update.
...
**Reframe:** Quarry already ships ~25 console pages, Cytoscape graphs, Monaco editor, React Flow playbooks, the 684-LOC Investigation Timeline, Responder PWA, ⌘J Copilot dock, ⌘K palette. The real UI work in v8.0 is five named features + polish.
...
**Goal:** make Quarry's eval the published reference. Everything reproducible from `make eval-public`.
...
**Acceptance:** Benchmark page shows Quarry scores on each dataset with date stamp + commit SHA.
...
### T6.1 — `app.quarry.dev` multi-tenant managed instance  (P0, L, depends on: T1.1, T2.2, T3.2, T3.3)
...
A landing page surfacing Quarry's deployment flexibility: air-gap mode, Ollama sidecar, on-prem, Helm, Terraform, any cloud, any country.

**File:** `apps/web/src/app/(marketing)/sovereign/page.tsx` with a country / cloud / deployment-mode matrix.

### T6.4 — `pnpm quarry:demo` polishing  (P1, S, depends on: —)
...
**Acceptance:** New user runs `pnpm quarry:demo`, gets to a populated dashboard with 4 cases in < 4 min on a warm laptop.
...
**New repo (or path):** `quarry-cursor-extension/` or `services/mcp/cursor-extension/`.
...
The maturity model is shipped in v6.0. Write it up as a white paper under the Quarry brand, host as a PDF at `apps/web/public/papers/l0-l4-automation-maturity.pdf`. Submit to MITRE D3FEND community channels.

### T7.3 — Three blog posts to anchor the narrative  (P1, S, depends on: T5.1, T7.1)

- "How Quarry investigates incidents in under a minute, with the numbers" — references T2 + T5 outputs
- "The SOC has an IDE now" — references T7.1 Cursor extension
- "Quarry owns the endpoint truth layer" — osctrl + FleetDM + `quarry-direct` + custom virtual tables + 16 osquery rules
...
| T6 | T6.1 `app.quarry.dev` waitlist live. T6.2 customer page template + first 2 case studies. |
...
[ ] T6.1  app.quarry.dev managed waitlist                  (P0, L)  → T1.1, T2.2, T3.2, T3.3
