#!/usr/bin/env tsx
/**
 * quarry:doctor — health-check for an Quarry dev environment.
 *
 * Verifies:
...
async function checkDocker() {
  ...
  if (!versionMatch) {
    ...
    if (majorNum < 2) {
      record(
        "docker compose v2",
        "FAIL",
        `Compose v${major}.${minor} detected — Quarry requires v2.0+`
      );
      return;
    }
  ...
  // running. The `quarry-` prefix on every container_name in both compose
  // files makes this a safe filter.
  const ps = tryRun("docker ps -a --format json --filter name=quarry-");
  if (!ps) {
    record(
      "containers running",
      "WARN",
      'no Quarry containers running — run `pnpm quarry:demo` (slim) or `docker compose up -d` (full)'
    );
    return;
  }
  ...
  if (containers.length === 0) {
    record(
      "containers running",
      "WARN",
      "no Quarry containers up — run `pnpm quarry:demo` (slim) or `docker compose up -d` (full)"
    );
    return;
  }
  ...
  const hasDemo = containers.some((c: any) =>
    typeof c.Names === "string" && c.Names.startsWith("quarry-demo-")
  );
  const hasFull = containers.some(
    (c: any) =>
      typeof c.Names === "string" &&
      c.Names.startsWith("quarry-") &&
      !c.Names.startsWith("quarry-demo-")
  );
  ...
  // The full stack uses `quarry-<role>` names; the demo stack uses
  // `quarry-demo-<role>`. We accept either prefix for the core services so
  // `pnpm quarry:demo` users don't see false FAILs.
  const expectedRoles = [
    "api",
    "agents",
    "web",
  ...
  for (const role of expectedRoles) {
    const candidates = [`quarry-${role}`, `quarry-demo-${role}`];
    ...
    if (!found) {
      ...
      record(
        `container ${role}`,
        "WARN",
        `not present (looked for ${candidates.join(" or ")}) — start the stack with \`pnpm quarry:demo\` or \`docker compose up -d\``
      );
      continue;
    }
...
// ---------- Run ----------
async function main() {
  console.log(c.bold("Quarry Doctor") + c.dim(" — pre-flight check"));
  await checkEnv();
  await checkDocker();
  await checkPorts();
  await checkApi();
  ...
  if (fails > 0) {
    console.log(
      c.red("\n  Quarry is not healthy. ") +
        "See the failing checks above. " +
        c.dim("Quickstart: https://github.com/beenuar/Quarry#quickstart")
    );
    process.exit(1);
  }
  if (warns > 0) {
    console.log(c.yellow("\n  Quarry is up but missing demo data or non-critical config."));
    process.exit(0);
  }
  console.log(c.green("\n  Quarry is healthy."));
  process.exit(0);
