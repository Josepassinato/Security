#!/usr/bin/env python3
"""
Quarry Public Eval — chart / markdown renderer
==============================================
...
def _render_wet_eval(report: dict[str, Any]) -> str:
    ...
    * Neither → emit a placeholder, never imputed numbers.
    ...
    return (
        ...
        "\n"
        "    export QUARRY_BENCH_PROVIDER=openai\n"
...
def _svg_open(width: int, height: int, *, title: str) -> list[str]:
    title_xml = (
        ...
        f'<desc>{html.escape("Generated by scripts/render_eval_charts.py — Quarry eval harness.")}</desc>\n  '
    )
    return [
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Render the Quarry eval JSON report into shareable markdown + SVG charts.",
    )
    parser.add_argument(
        "report",
        nargs="?",
