#!/usr/bin/env tsx
/**
 * quarry:acceptance — WS-A acceptance gate harness.
 *
 * Wraps `pnpm quarry:demo` with strict clone-to-investigation budget
 ...
 *   3. Run `pnpm quarry:demo --no-open --budget-ms <ms> --results-file <tmp>`
 *      which:
 *        - verifies Docker
 *        - pulls images
 *        - boots the stack
 ...
 *   4. Append the report to .quarry/acceptance-history.jsonl as a single line so
 ...
 *
 * Usage:
 *   pnpm quarry:acceptance                  # default 5-minute budget, warm start
 *   pnpm quarry:acceptance --cold           # prune images first (true clean Mac)
 *   pnpm quarry:acceptance --budget-ms 360000   # 6 minutes (for slower CI)
 *   pnpm quarry:acceptance --history-only   # just print the trend, no run
 *
 * Why a separate script (and not just `quarry:demo --budget-ms`)?
 *   - quarry:demo is the developer ergonomic tool; the user expects it to
 ...
 *   - tsx is already a dev dep and used by quarry:demo / quarry:doctor.
...
const HISTORY_DIR = join(ROOT, ".quarry");
...
function printHelp(): void {
  console.log(`quarry:acceptance — WS-A acceptance gate harness

Usage: pnpm quarry:acceptance [flags]
...
Flags:
  ...
  --tag <tag>           Override QUARRY_TAG (default latest)
...
function pruneImages(tag: string): void {
  console.log(c.dim(`  pruning quarry images for tag=${tag} (cold start)...`));
  // We deliberately target only ghcr.io/quarry-community/* images for the
  ...
  const list = spawnSync(
    ...
    [
      "images",
      `ghcr.io/quarry-community/quarry-*:${tag}`,
      "--quiet",
    ],
    { encoding: "utf8" },
  );
...
function runDemo(flags: Flags, resultsFile: string): number {
  ...
  const result = spawnSync("pnpm", ["exec", ...args], {
    cwd: ROOT,
    stdio: "inherit",
    env: { ...process.env, QUARRY_TAG: flags.tag },
  });
...
function writeHistoryEntry(entry: HistoryEntry): void {
  ...
  // data. If this ever becomes a problem, .quarry/ is gitignored anyway.
  appendFileSync(HISTORY_FILE, JSON.stringify(entry) + "\n");
}

function printHistoryTrend(): void {
  if (!existsSync(HISTORY_FILE)) {
    console.log(c.dim("no acceptance history yet — run `pnpm quarry:acceptance` once."));
    return;
  }
...
async function main(): Promise<void> {
  ...
  //    restart" which is what quarry:doctor cares about, not WS-A.
  tearDownStack();
  ...
  const resultsFile = `/tmp/quarry-acceptance-${Date.now()}.json`;
  ...
  console.log(c.dim("  view trend: pnpm quarry:acceptance --history-only\n"));
