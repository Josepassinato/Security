#!/usr/bin/env tsx
/**
 * quarry:demo — single-command path to a running demo stack.
 *
 * Steps:
...
interface Flags {
  ...
  // a structured JSON timing report that scripts/quarry-acceptance.ts
...
function printHelp(): void {
  ...
  const exe = "pnpm quarry:demo";
  console.log(`${c.bold("Quarry Demo")} — one-command path to a running demo stack.
...
// ---------- Port allocation ----------
//
// Why this exists: every prior failed `pnpm quarry:demo` run can leave
...
// values via QUARRY_*_PORT env vars (with defaults), so manual `docker
...
// we only honor the explicit QUARRY_NO_BROWSER opt-out.
function isHeadless(): boolean {
  if (process.env.QUARRY_NO_BROWSER === "1") return true;
...
function portEnv(ports: PortMap): NodeJS.ProcessEnv {
  return {
    QUARRY_WEB_PORT: String(ports.web),
    QUARRY_API_PORT: String(ports.api),
    QUARRY_REALTIME_PORT: String(ports.realtime),
    QUARRY_POSTGRES_PORT: String(ports.postgres),
    QUARRY_REDIS_PORT: String(ports.redis),
    QUARRY_KAFKA_PORT: String(ports.kafka),
  };
}

async function startStack(flags: Flags): Promise<boolean> {
  step(3, 7, "Starting Quarry demo stack");

  // Pick host ports BEFORE compose up so any conflict (lingering
  // quarry-demo-* container, unrelated dev server, local Postgres) is
...
async function openInBrowser(
  ...
  if (flags.noOpen) {
    ...
    // CI, headless server, or QUARRY_NO_BROWSER=1. Don't try to spawn a
    // GUI process the user can't see — just leave the URL in the banner.
    log(c.dim("headless environment detected — not launching browser (set QUARRY_NO_BROWSER=0 to override)"));
  } else {
    openBrowser(url);
  }

  console.log(`
${c.bold(c.green("Quarry demo is up."))}
  ${c.bold("Web:")}        ${url}
...
${c.dim("Useful commands:")}
  pnpm quarry:doctor                           ${c.dim("# health check")}
...
async function main() {
  ...
  if (!(await waitForHealth())) {
    console.error(c.red("\nstack failed to come up healthy. Run `pnpm quarry:doctor` for details."));
    process.exit(1);
  }
  if (!seedData(flags)) {
  ...
  // and budget verdict come immediately after — for a `pnpm quarry:demo`
