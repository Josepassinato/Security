...
func writeManifest(t *testing.T, dir string, content map[string]any) {
	...
	if err := os.WriteFile(filepath.Join(dir, "quarry-plugin.json"), data, 0o600); err != nil {
...
func TestLoadManifestValid(t *testing.T) {
	...
	if m.PluginType != quarry.PluginTypeEnricher {
...
func TestLoadManifestMissingFile(t *testing.T) {
	dir := t.TempDir()
	_, err := quarry.LoadManifest(dir)
	if err == nil {
		t.Fatal("expected error for missing manifest")
	}
	var loaderErr *quarry.LoaderError
...
func TestLoadManifestInvalidJSON(t *testing.T) {
	dir := t.TempDir()
	if err := os.WriteFile(filepath.Join(dir, "quarry-plugin.json"), []byte("not-json"), 0o600); err != nil {
		t.Fatal(err)
	}
	_, err := quarry.LoadManifest(dir)
	if err == nil {
...
func TestLoadManifestActionType(t *testing.T) {
	...
	if m.PluginType != quarry.PluginTypeAction {
