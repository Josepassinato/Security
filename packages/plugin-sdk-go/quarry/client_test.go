...
func pctxFor(baseURL string) quarry.PluginContext {
	return quarry.PluginContext{APIBaseURL: baseURL, APIToken: "test-token"}
}
...
func TestClientGetCase(t *testing.T) {
	...
	client := quarry.NewClient(pctxFor(srv.URL), 0)
...
func TestClientErrorOnNon2xx(t *testing.T) {
	...
	var clientErr *quarry.ClientError
	if cerr, ok := err.(*quarry.ClientError); ok {
		clientErr = cerr
	}
	if clientErr == nil {
