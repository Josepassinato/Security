"""QuarryClient — async httpx-based client for the Quarry REST API.

Usage::

    async with QuarryClient(base_url="https://soc.example.com", token="aisoc_...") as c:
...
class QuarryError(Exception):
    """Raised when the Quarry API returns a non-2xx response."""

    def __init__(self, status_code: int, detail: str) -> None:
        self.status_code = status_code
        self.detail = detail
        super().__init__(f"Quarry API {status_code}: {detail}")
    ...
    def _raise(r: httpx.Response) -> None:
        if not r.is_success:
            try:
                ...
                detail = r.text
            raise QuarryError(r.status_code, detail)
...
class QuarryClient:
    """Async Python client for the Quarry REST API.
    ...
    Or manage the lifecycle manually::
        ...
        client = QuarryClient(base_url="...", token="...")
    ...
    async def __aenter__(self) -> "QuarryClient":
    ...
    async def graphql(
        ...
        if self._http is None:
            raise RuntimeError("Use QuarryClient as an async context manager")
        r = await self._http.post("/graphql", json={"query": query, "variables": variables})
        if not r.is_success:
            raise QuarryError(r.status_code, r.text)
