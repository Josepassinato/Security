"""Async HTTP client for interacting with the Quarry REST API.

Plugins can use ``QuarryClient`` to:
...
class QuarryClientError(Exception):
    """Raised when the Quarry API returns an unexpected response."""

    def __init__(self, status_code: int, body: str) -> None:
        self.status_code = status_code
        self.body = body
        super().__init__(f"Quarry API error {status_code}: {body}")
...
class QuarryClient:
    ...
    Usage::
        ...
        async with QuarryClient(ctx) as client:
    ...
    def _client_instance(self) -> httpx.AsyncClient:
        if self._http is None:
            raise RuntimeError("QuarryClient must be used as an async context manager.")
        return self._http
    ...
    def _raise_for_status(resp: httpx.Response) -> None:
        if resp.is_error:
            raise QuarryClientError(resp.status_code, resp.text)
