"""Base plugin abstractions for Quarry plugins."""
...
class PluginContext(BaseModel):
    ...
    api_base_url: str = Field(..., description="Quarry API base URL, e.g. 'http://api:8000'")
    api_token: str = Field(..., description="Scoped API token for this plugin")
    config: dict[str, Any] = Field(
        default_factory=dict,
        description="Plugin-level config from quarry-plugin.yaml",
    )
...
class QuarryPlugin(ABC):
    """Base class for all Quarry plugins."""
