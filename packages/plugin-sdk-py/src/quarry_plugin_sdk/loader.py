...
Reads ``quarry-plugin.yaml`` from a plugin package directory, validates it
...
from .plugin import QuarryPlugin, PluginManifest
...
def load_manifest(plugin_dir: Path) -> PluginManifest:
    """Parse and validate ``quarry-plugin.yaml`` inside *plugin_dir*.

    Args:
        plugin_dir: Directory that contains ``quarry-plugin.yaml``.
    ...
    manifest_path = plugin_dir / "quarry-plugin.yaml"
    if not manifest_path.exists():
        raise PluginLoadError(f"No quarry-plugin.yaml found in {plugin_dir}")
...
def load_plugin_from_directory(plugin_dir: Path) -> QuarryPlugin:
    """Load a plugin from a directory.

    The directory must contain:
    - ``quarry-plugin.yaml`` — manifest file
    - ``plugin.py`` (or package) — must expose a ``create_plugin()`` factory
      that returns an :class:`QuarryPlugin` instance.

    Args:
        plugin_dir: Path to the plugin directory.

    Returns:
        An instantiated :class:`QuarryPlugin`.
    ...
    if factory is None:
        raise PluginLoadError(
            ...
            "that returns an QuarryPlugin instance."
        )

    try:
        plugin: QuarryPlugin = factory()
    except Exception as exc:
        raise PluginLoadError(
            f"create_plugin() in {entry_point} raised an error: {exc}"
        ) from exc

    if not isinstance(plugin, QuarryPlugin):
        raise PluginLoadError(
            f"create_plugin() must return an QuarryPlugin, got {type(plugin)}"
        )
