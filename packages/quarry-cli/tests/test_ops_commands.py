...
def test_top_level_help_lists_new_groups(runner: CliRunner) -> None:
    ...
    # All four entrypoints must surface under `quarry --help`.
...
def test_serve_invokes_docker_compose_up(
    ...
    """``quarry serve`` shells out to ``docker compose -f <file> up -d``."""
...
def test_db_upgrade_invokes_run_migrations(
    ...
    """``quarry db upgrade`` execs the custom forward-only migration runner."""
    captured: dict[str, object] = {}
...
def test_mcp_serve_falls_back_to_npx(
    ...
    """When no dist build exists, the CLI uses ``npx @quarry/mcp``."""
