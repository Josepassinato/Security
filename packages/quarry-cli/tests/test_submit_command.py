"""CLI surface tests for ``quarry submit``.

These tests pin the contract the founder-flow quickstart relies on:

* ``quarry submit <file>`` is registered and discoverable via ``--help``.
...
from quarry_cli import main as cli_main
from quarry_cli.main import cli
...
def test_submit_posts_to_alerts_submit_endpoint(
    ...
    assert body["connector_id"] == "quarry-cli-submit"
...
def test_submit_env_overrides_default_api_url(
    ...
    fixture_path = _write_fixture(tmp_path, {"events": [{"uuid": "x"}]})
    monkeypatch.setenv("QUARRY_API_URL", "http://api-worker:8000")
...
def test_submit_returns_nonzero_on_4xx(
    ...
    assert "Quarry API returned 400" in result.output
...
def test_submit_returns_nonzero_on_transport_error(
    ...
    result = runner.invoke(cli, ["submit", str(fixture_path)])
    assert result.exit_code == 1, result.output
    assert "Quarry API unreachable" in result.output
    assert "quarry serve" in result.output
