"""
Quarry CLI — scaffold, validate, publish, and operate.

Commands:
  quarry plugin new <name>           Scaffold a typed plugin from disk templates
  quarry plugin scaffold <name>      Alias for `plugin new` (backwards compat)
  quarry plugin validate [path]      Validate plugin.yaml against JSON Schema
  quarry plugin publish [path]       Sign with Ed25519 key + POST to Quarry API
  quarry detection validate <file>   Validate Sigma rule syntax
  quarry keygen                      Generate an Ed25519 signing key pair
  quarry serve [--detach]            Start the dev stack via docker compose
  quarry db upgrade                  Run database migrations against the dev stack
  quarry mcp serve [--transport]     Launch the MCP server for IDE assistants
  quarry mcp install --host <h>      Wire Quarry into Claude / Cursor / Continue
  quarry submit <file>               POST an alert/event JSON payload to the Quarry API
"""
...
def _templates_root() -> Path:
    ...
    """
    pkg_files = resources.files("quarry_cli") / "templates"
...
@click.group()
@click.version_option(package_name="quarry-cli")
def cli() -> None:
    """Quarry Developer CLI — build, validate, and publish plugins & detections."""
...
def _scaffold_plugin(name: str, output_dir: str, plugin_type: str, author: str) -> Path:
    ...
    console.print(
        Panel(
            ...
            f"  3. Run [bold]quarry plugin validate {out}[/bold] to check",
...
def plugin_scaffold(name: str, output_dir: str, plugin_type: str, author: str) -> None:
    """Alias for ``quarry plugin new`` (kept for backwards compatibility)."""
    _scaffold_plugin(name, output_dir, plugin_type, author)
...
def plugin_validate(path: str) -> None:
    """Validate plugin.yaml against the Quarry manifest schema."""
...
@click.option(
    "--api-url",
    envvar="QUARRY_API_URL",
    default="http://localhost:8000",
    show_default=True,
    help="Quarry API base URL",
)
@click.option(
    "--api-key",
    envvar="QUARRY_API_KEY",
    required=True,
    help="Quarry API key (or set QUARRY_API_KEY env var)",
)
@click.option(
    "--private-key",
    envvar="QUARRY_SIGNING_KEY",
    default="~/.quarry/signing.key",
    show_default=True,
    help="Path to Ed25519 private key PEM file",
)
def plugin_publish(path: str, api_url: str, api_key: str, private_key: str) -> None:
    """Sign and publish a plugin to the Quarry community marketplace."""
    import io
    import tarfile
    ...
    if not key_path.exists():
        ...
        console.print("Run [bold]quarry keygen[/bold] to generate a key pair.")
        sys.exit(1)
...
def keygen(output_dir: str) -> None:
    ...
    console.print(
        Panel(
            ...
            f"Register your public key with the Quarry marketplace before publishing:\n"
            f"  [bold]quarry plugin publish --private-key {priv_path} <plugin-dir>[/bold]",
...
def serve(detach: bool, build: bool) -> None:
    """Start the Quarry dev stack via docker compose.
    ...
    console.print(
        Panel(
            ...
            title="[bold green]quarry serve[/bold green]",
        )
    )
...
def db_upgrade(service: str) -> None:
    ...
    console.print(
        Panel(
            f"[bold]cwd:[/bold] {repo_root}\n"
            f"[bold]cmd:[/bold] {' '.join(cmd)}",
            title="[bold green]quarry db upgrade[/bold green]",
        )
    )
    result = subprocess.run(cmd, cwd=str(repo_root))
    if result.returncode != 0:
        console.print(
            "[yellow]If the stack is not up yet, run [bold]quarry serve[/bold] "
...
def _mcp_argv(repo_root: Path, subcommand: str, extra: list[str]) -> tuple[list[str], str]:
    ...
    return (
        [npx, "@quarry/mcp", subcommand, *extra],
        f"npx @quarry/mcp {subcommand} {' '.join(extra)}".strip(),
    )
...
def mcp_serve(transport: str, port: int | None) -> None:
    """Launch the MCP server that exposes Quarry to IDE assistants.

    Prefers the locally built ``services/mcp/dist/index.js`` (from
    ``pnpm --filter @quarry/mcp build``). Falls back to ``npx @quarry/mcp``
    ...
    console.print(
        Panel(
            ...
            title="[bold green]quarry mcp serve[/bold green]",
        )
    )
...
@click.option(
    ...
    required=True,
    help="IDE assistant to wire Quarry into.",
)
def mcp_install(host: str) -> None:
    """Register the Quarry MCP server with the given IDE assistant.

    Thin wrapper over ``quarry-mcp install --host <host>`` that picks the local
    dist build when available and falls back to ``npx @quarry/mcp`` otherwise.
    """
    repo_root = _find_repo_root()
    ...
    console.print(
        Panel(
            f"[bold]cwd:[/bold] {repo_root}\n"
            f"[bold]cmd:[/bold] {label}",
            title="[bold green]quarry mcp install[/bold green]",
        )
    )
...
# ── submit command ───────────────────────────────────────────────────────────
#
# `quarry submit <file>` lets the founder-flow demo and any operator land a
...
# ``--ingest-url`` flag and ``QUARRY_INGEST_URL`` env var as deprecated aliases
# for ``--api-url`` / ``QUARRY_API_URL`` so existing demo scripts and the v1.0
# video voiceover keep working. The tenant id, when not derivable from the
# bearer token (dev mode bypass), comes from ``--tenant-id`` /
# ``QUARRY_TENANT_ID`` and is still echoed in the request panel.

_DEFAULT_API_URL = "http://127.0.0.1:8000"
_DEFAULT_TENANT_ID = "00000000-0000-0000-0000-000000000001"
_DEFAULT_CONNECTOR_ID = "quarry-cli-submit"
...
def _resolve_api_url(api_url: str, ingest_url: str | None) -> str:
    ...
    Precedence (highest first):
      1. ``--ingest-url`` / ``QUARRY_INGEST_URL`` (deprecated, prints a warning)
      2. ``--api-url`` / ``QUARRY_API_URL``
      3. default ``http://127.0.0.1:8000``
    """
    if ingest_url:
        console.print(
            "[yellow]--ingest-url / QUARRY_INGEST_URL is deprecated; "
            "use --api-url / QUARRY_API_URL instead.[/yellow]"
        )
        return ingest_url
    return api_url
...
@click.option(
    ...
    show_default=True,
    help="Base URL for the Quarry API (e.g. http://127.0.0.1:8000).",
)
@click.option(
    "--ingest-url",
    envvar="QUARRY_INGEST_URL",
    default=None,
    show_default=False,
...
@click.option(
    ...
    show_default=False,
    help=(
        "Quarry bearer token. Optional in dev mode (the API resolves an "
        "unauthenticated request to the demo tenant)."
    ),
)
@click.option(
    "--tenant-id",
    envvar="QUARRY_TENANT_ID",
...
def submit(
    ...
    """Submit a JSON alert/event payload to the Quarry API.
    ...
    console.print(
        Panel(
            ...
            title="[bold green]quarry submit[/bold green]",
        )
    )
    ...
    try:
        ...
        console.print(
            f"[red]Quarry API unreachable at {url}:[/red] {exc}\n"
            "Is the dev stack running? Try [bold]quarry serve[/bold] first."
        )
        sys.exit(1)

    if resp.status_code >= 400:
        console.print(
            f"[red]Quarry API returned {resp.status_code}:[/red] {resp.text[:500]}"
        )
        sys.exit(1)
    ...
    try:
        ...
        console.print(
            f"[red]Quarry API returned non-JSON ({resp.status_code}):[/red] "
            f"{resp.text[:200]}"
        )
        sys.exit(1)
