...
description: How Quarry handles authentication, authorization, multi-tenant isolation, audit logging, and secrets — the controls operators care about.
---

# Security model

This page is the operator-facing summary of how Quarry protects the data flowing through it. If you're evaluating Quarry for a regulated environment, this is the page to read first; if you're already running it, this is the reference for the controls you have available.
...
Quarry supports two enterprise SSO protocols out of the box:

- **OIDC** — configured via `services/api/app/auth/oidc.py`. Point Quarry at your IdP's discovery URL, set the client ID/secret, and map IdP groups to Quarry roles in the role mapping config. Common IdPs tested: Okta, Entra ID (Azure AD), Google Workspace, Auth0.
...
For programmatic clients (CI runners, external integrations, scripts) Quarry issues scoped API keys:
...
Generation logic: [`generate_api_key()` in `services/api/app/core/security.py`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/core/security.py).
...
Defined in [`ROLE_PERMISSIONS`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/core/security.py):

| Role | Intended user | Notable permissions |
|---|---|---|
| `platform_admin` | Quarry operator (you) | `*` — every permission |
...
When a user hits an endpoint they don't have permission for, Quarry returns `403 Forbidden` with the missing permission name in the body. It does **not** return `404` to hide existence — the resource ID is already in the URL the caller chose, so hiding it offers no real protection and complicates support.

## Multi-tenant isolation (RLS)

Quarry is multi-tenant by design. Every tenant-partitioned table has Postgres Row-Level Security enforced. The migration that sets this up is [`002_rls.sql`](https://github.com/Josepassinato/quarry/blob/main/services/api/migrations/002_rls.sql).
...
Every state-changing API action is appended to an immutable audit log. The schema lives in [`004_audit_log.sql`](https://github.com/Josepassinato/quarry/blob/main/services/api/migrations/004_audit_log.sql) (chain columns added in [`043_audit_log_hash_chain.sql`](https://github.com/Josepassinato/quarry/blob/main/services/api/migrations/043_audit_log_hash_chain.sql)), the model in `services/api/app/models/audit.py`, and the helper that emits events in [`services/api/app/services/audit.py`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/services/audit.py).
...
| `actor_ip` | Resolved by `resolve_client_ip()` — direct TCP peer by default; `X-Forwarded-For` is consulted **only** if the peer is in `QUARRY_TRUSTED_PROXIES` |
| `action` | Dot/colon string — e.g. `cases:create`, `connectors:delete`, `playbooks:execute` |
| `resource`, `resource_id` | What was touched |
| `changes` | Before/after delta JSON, **redacted** by `redact_changes()` before persistence (secrets, tokens, passwords masked; capped at `QUARRY_AUDIT_MAX_CHANGES_BYTES`) |
...
* `QUARRY_TRUSTED_PROXIES` is **empty by default** → `X-Forwarded-For` is ignored entirely and the direct TCP peer is recorded.
...
Configure `QUARRY_TRUSTED_PROXIES` to the CIDR(s) of your ingress / load balancer in production. The trusted-proxy resolver lives in [`services/api/app/core/trusted_proxy.py`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/core/trusted_proxy.py).

### `changes` redaction and size cap

The `changes` payload often holds before/after snapshots of user objects, settings, or credentials. We sanitize it on the write path in [`services/api/app/services/audit_redaction.py`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/services/audit_redaction.py):
...
* The serialized JSON is capped at `QUARRY_AUDIT_MAX_CHANGES_BYTES` (default 65,536). Over-sized payloads are replaced with a `{ "_truncated": true, "_size": <bytes> }` marker rather than persisted — this stops a single bad caller from ballooning the audit table.
...
Quarry treats LLM prompts as **not a trust boundary** and defends in layers. The sanitiser lives in [`services/agents/app/investigator/prompt_sanitizer.py`](https://github.com/Josepassinato/quarry/blob/main/services/agents/app/investigator/prompt_sanitizer.py) and every investigator agent calls it on the context it hands to the model.
...
This is **defence in depth**, not a guarantee — there is no way to make prompt injection impossible while still letting LLMs read attacker-controlled telemetry. The combination of redaction + length caps + explicit framing + schema validation has so far defeated every payload in our test suite ([`services/agents/tests/test_prompt_sanitizer.py`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_prompt_sanitizer.py)). For high-stakes deployments, also:
...
| Signature-before-copy | `_verify_plugin_signature()` runs against the temp dir *before* anything is copied into `QUARRY_PLUGINS_DIR`. In `strict` mode an unsigned/tampered image is rejected and the temp dir is cleaned up — no malicious `plugin.py` ever lands somewhere the runtime would later import it. |
...
Quarry is HTTP-first. The expected production deployment terminates TLS at an ingress (nginx, Envoy, ALB, Cloud Run, …) and forwards plaintext to the API service over a private network. The API trusts `X-Forwarded-For` and `X-Forwarded-Proto` for IP attribution and HTTPS-redirect logic; configure your ingress to strip and replace those headers from external traffic.

For service-to-service traffic between the API, ingest, fusion, and agents, mTLS via a service mesh (Istio, Linkerd, Consul Connect) is the recommended posture. Quarry does not ship its own mesh.
...
- **No per-service drift.** Adding a new console subdomain means setting `QUARRY_CORS_ORIGINS` once at the deployment layer — no code change in any service.
...
- Only `http://` and `https://` are allowed by default (override with `QUARRY_SSRF_ALLOWED_SCHEMES` if you genuinely need `https` only or an additional scheme).
...
- Cloud metadata endpoints — `169.254.169.254`, `fd00:ec2::254`, `metadata.google.internal`, `metadata.azure.com`, `metadata`, `metadata.aws` — are blocked even if `QUARRY_SSRF_ALLOW_PRIVATE=true`.
- Operators can extend the deny list with `QUARRY_SSRF_EXTRA_BLOCKED_HOSTS=internal-only.example.com,10.0.0.5`.

If a playbook needs to reach a private webhook (Slack on a private network, an internal Jira, etc.), set `QUARRY_SSRF_ALLOW_PRIVATE=true` **only** on the agents service and keep network-level egress controls in place. The metadata block list always applies.
...
When you move from `pnpm quarry:demo` to a production deployment, walk through this list:

- [ ] Rotate `SECRET_KEY` and `QUARRY_CREDENTIAL_KEY` to fresh, randomly generated values stored in your secret manager.
...
- [ ] Configure `QUARRY_TRUSTED_PROXIES` to the CIDR(s) of your ingress / load balancer so `actor_ip` is sourced from `X-Forwarded-For` instead of the immediate TCP peer. Leave empty if the API is exposed directly to clients.
...
- [ ] Set `QUARRY_CORS_ORIGINS` to an explicit allow-list (your console domains) and `QUARRY_ENV=production`. Confirm services refuse to start if the allow-list contains `*` — that's the wildcard guard doing its job.
- [ ] Enable mTLS between services if you're running on Kubernetes with a mesh.
- [ ] Subscribe to the Quarry GitHub Security Advisories for vulnerability notifications.
...
The remaining alert categories (`py/uninitialized-local-variable`, `py/side-effect-in-assert`, `py/incomplete-url-substring-sanitization`, `py/ineffectual-statement`, `py/unnecessary-lambda`, `py/mixed-returns`, `py/unused-global-variable`, `py/unused-import`) are all standard Python correctness items and the fixes were uncontroversial — see the `[Unreleased]` section of the [CHANGELOG](https://github.com/Josepassinato/quarry/blob/main/CHANGELOG.md) for the per-PR breakdown.

## Reporting a vulnerability

Security issues should be reported privately via [GitHub Security Advisories](https://github.com/Josepassinato/quarry/security/advisories/new), not as public issues. We aim to acknowledge reports within 2 business days and ship a coordinated disclosure with the reporter. The [Contributing guidelines](../contributing/guidelines) cover the full process.
