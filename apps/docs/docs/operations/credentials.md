...
description: How Quarry stores connector credentials at rest, how to rotate the master key, and the roadmap for hosted OAuth.
---

# Credential vault and secret management

Connectors need real secrets — Azure client secrets, GCP service-account keys, GitHub tokens. Quarry stores them with a defense-in-depth design: **encrypted at the application layer, with the database as a transport medium, not a trust boundary**.
...
| **Master key** | `QUARRY_CREDENTIAL_KEY` environment variable, 32 url-safe base64 bytes |
| **Implementation** | [`services/api/app/security/credential_vault.py`](https://github.com/Josepassinato/quarry/blob/main/services/api/app/security/credential_vault.py) |
...
- A leaked `pg_dump` is useless without `QUARRY_CREDENTIAL_KEY`.
...
```bash
export QUARRY_CREDENTIAL_KEY="kY7w…=" # never commit this value
```
...
|---|---|
| Kubernetes | A `Secret` named `quarry-credential-vault`, mounted as env into both `quarry-api` and `quarry-connectors` Deployments |
| Fly.io | `fly secrets set QUARRY_CREDENTIAL_KEY=…` in **both** apps |
...
For higher-assurance deployments, treat `QUARRY_CREDENTIAL_KEY` as a long-lived encryption key and protect it with an external KMS. Two patterns work:
...
Independent of the master key, the upstream credentials inside each connector instance should be rotated on the cadence of your identity provider's policy (typically 90–180 days for OAuth client secrets and PATs). The Quarry UI supports in-place editing: the **Configure** action on a connector card opens the schema-driven form pre-populated with masked values, lets you paste a new secret, and re-encrypts on save. No restart, no scheduler downtime.
...
(BYOK): each tenant can point Quarry at its own OpenAI account, Azure
...
1. Reads the env baseline (`OPENAI_*` / `LLM_*` / `QUARRY_LLM_MODEL`).
...
4. Applies the air-gap rule (`QUARRY_AIRGAPPED=true` blocks
   `api.openai.com` even with a valid tenant key, but allows private
   gateways like `litellm.internal:4000`).

The agents-side vault is read-only. If `QUARRY_CREDENTIAL_KEY` is
...
Several connectors (Azure, GCP, Google Workspace) currently require the operator to do an Azure-AD-app or service-account dance before they can be added in Quarry. This is fine for SOC engineers but a friction point for everyone else. Hosted OAuth is the planned solution:
...
- **Phase 2 (planned)**: Quarry Cloud (and self-hosted with an OAuth-app config) will surface a one-click flow that exchanges an authorization code for an offline refresh token, encrypts it via the vault, and stores it as the connector's `auth_config`.
...
| Threat | Mitigation |
|---|---|
| Stolen DB backup | Useless without `QUARRY_CREDENTIAL_KEY` |
...
- [Environment Variables](/docs/deployment/env-vars) — full env reference including `QUARRY_CREDENTIAL_KEY` and related settings.
