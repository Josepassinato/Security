...
description: How the Quarry alert-fusion substrate collapses a noisy 1,000-alert stream into ~247 incidents while losing zero critical alerts. Methodology, latest measured numbers, reproduction, and CI artefact location.
---

# Alert Reduction

> **Synthetic, deterministic, reproducible in under a second.** This page
> documents the Quarry alert-reduction benchmark — what it measures, how the
...
> rates (tracked under [issue #5](https://github.com/Josepassinato/quarry/issues/5)).
...
[`services/agents/tests/test_alert_reduction.py`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_alert_reduction.py)
...
[`generate_noisy_alert_stream`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_alert_reduction.py)
...
in [`fuse_alerts`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_alert_reduction.py)
inside the test file (so the benchmark stays self-contained) and mirrors the
production logic in
[`services/fusion`](https://github.com/Josepassinato/quarry/tree/main/services/fusion).
...
[`test_critical_alerts_never_dropped`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_alert_reduction.py)
...
The
[`test_run_is_deterministic`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/test_alert_reduction.py)
...
```bash
curl -s https://raw.githubusercontent.com/beenuar/Quarry/eval-results/eval/results/latest.json \
...
- **It does not compute per-rule false-positive rates.** That is the next
  ...
  [issue #5 — per-rule cross-fire FP eval gate](https://github.com/Josepassinato/quarry/issues/5)
  ([PR #89](https://github.com/Josepassinato/quarry/pull/89), in flight). Once that
...
- **Per-rule FP rate breakdown** — depends on
  [issue #5](https://github.com/Josepassinato/quarry/issues/5) /
  [PR #89](https://github.com/Josepassinato/quarry/pull/89). Will surface per-rule
...
- [Synthetic telemetry corpus](../benchmark.md#5-synthetic-telemetry-corpus) —
  ...
  against.
- [`scripts/run_evals.py`](https://github.com/Josepassinato/quarry/blob/main/scripts/run_evals.py) —
