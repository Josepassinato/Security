...
- **osquery endpoint telemetry** — native osquery TLS server (`services/osquery-tls/`) gives Linux servers, containers, and cloud workloads first-class coverage without a commercial EDR agent. Ships with 5 curated packs (discovery, process-events, network-events, browser-extensions, FIM), a tenant-aware pack resolver, a FIM pipeline (ingest → query → React dashboard), and 4 FIM detection rules. The companion [osquery extensions binary](./connectors/osquery-extensions) adds 5 virtual tables (`aisoc_pending_actions`, `aisoc_alert_cache`, `aisoc_attck_persistence`, `aisoc_kernel_modules_verified`, `aisoc_browser_extensions`) to any osquery instance, built and signed via a cross-platform CI matrix.
