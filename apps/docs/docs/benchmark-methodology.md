...
description: How Quarry's public eval is built — dataset, substrate vs wet eval, rate card, exact reproduction commands, and the threats to validity we are honest about. The reference document for anyone reproducing the numbers on the benchmark page or comparing Quarry against their own SOC.
---

# Benchmark methodology

This page is the open reference for Quarry's public evaluation. It exists so that
...
— the dataset, the rubric, and the runner are private. Quarry takes the
...
[`services/agents/tests/eval_data/synthetic_incidents.json`](https://github.com/Josepassinato/quarry/blob/main/services/agents/tests/eval_data/synthetic_incidents.json).
The corpus is regenerated by [`scripts/generate_eval_incidents.py`](https://github.com/Josepassinato/quarry/blob/main/scripts/generate_eval_incidents.py)
...
| Driver            | [`scripts/run_evals.py`](https://github.com/Josepassinato/quarry/blob/main/scripts/run_evals.py) with `--wet` (added by T2.4) |
| Dataset           | Same `synthetic_incidents.json` (deterministic) |
| LLMs              | Configurable via env (`QUARRY_BENCH_PROVIDER=openai\|anthropic\|ollama`) |
| Telemetry captured | Wall-clock latency p50/p95/p99, prompt + completion tokens, USD cost |
| CI gate           | [`.github/workflows/wet-eval-weekly.yml`](https://github.com/Josepassinato/quarry/blob/main/.github/workflows/wet-eval-weekly.yml) (T5.5) |
...
Cross-link: [`wet-eval-weekly.yml`](https://github.com/Josepassinato/quarry/blob/main/.github/workflows/wet-eval-weekly.yml)
...
> **Rates evolve.** Providers update prices, and Quarry ships a new wet-eval
...
[`package.json`](https://github.com/Josepassinato/quarry/blob/main/package.json).
...
[`apps/docs/docs/operations/credentials.md`](https://github.com/Josepassinato/quarry/blob/main/apps/docs/docs/operations/credentials.md).
...
[`eval-results`](https://github.com/Josepassinato/quarry/tree/eval-results) branch.
...
3. **Run the same five suites you can reproduce.** `mitre_accuracy`,
   ...
   [`services/agents/tests/`](https://github.com/Josepassinato/quarry/tree/main/services/agents/tests)
   and require no Quarry-specific runtime. Replace the substrate code under
...
We welcome any reproduction — including ones that show Quarry losing on a
...
What the eval **can** tell you: whether Quarry's substrate gets worse over
...
single most-misread part of the eval. If you want to claim "Quarry's agent
...
- The [eval-results branch](https://github.com/Josepassinato/quarry/tree/eval-results)
  for every historic snapshot.
- The [v8.0 north-star plan](https://github.com/Josepassinato/quarry/blob/main/plans/future-release-implementation-plan.md)
...
- Open-source agent code lives under
  [`services/agents/`](https://github.com/Josepassinato/quarry/tree/main/services/agents) —
