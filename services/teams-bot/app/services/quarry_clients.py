...
ships :class:`services.slack-bot.app.services.aisoc_clients.QuarryActionsClient`
which is parameterised by env vars and works for any caller. In a real
deployment the Teams bot pulls in ``quarry-slack-bot`` as a sibling
...
class _FallbackActionsClient:
    ...
    def __init__(self) -> None:
        self._base_url = os.environ.get("QUARRY_ACTIONS_BASE_URL", "http://quarry-actions:8085").rstrip("/")
        self._token = os.environ.get("QUARRY_ACTIONS_SERVICE_TOKEN", "") or os.environ.get("QUARRY_API_SERVICE_TOKEN", "")
        self._tenant = os.environ.get("QUARRY_DEFAULT_TENANT_ID", "00000000-0000-0000-0000-000000000000")
...
def build_actions_client() -> _ActionsClient:
    ...
    if slack_module is not None and hasattr(slack_module, "QuarryActionsClient"):
        try:
            return slack_module.QuarryActionsClient.from_settings()  # type: ignore[no-any-return]
