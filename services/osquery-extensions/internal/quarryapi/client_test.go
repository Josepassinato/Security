...
func newTestClient(t *testing.T, handler http.HandlerFunc) (*quarryapi.Client, *httptest.Server) {
	t.Helper()
	...
	return quarryapi.New(cfg), srv
}

func TestGetPendingActions(t *testing.T) {
	want := []quarryapi.PendingAction{
		{
			ActionID:   "act-001",
			ActionType: "isolate",
			Target:     "test-host",
...
func TestGetAlertCache(t *testing.T) {
	want := []quarryapi.AlertCacheEntry{
