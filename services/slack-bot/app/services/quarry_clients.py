"""
HTTP clients for the Quarry backend services.
...
* :class:`QuarryApiClient` — proxies ``/api/v1/cases/*`` endpoints used by the
  ``/quarry list``, ``/quarry investigate`` and ``/quarry explain`` commands.
* :class:`QuarryActionsClient` — submits and approves response actions used by
  ``/quarry isolate`` and ``/quarry block``.
...
* Surface failures as :class:`QuarryClientError` so command handlers can render
...
class QuarryClientError(RuntimeError):
    """
    Raised when an Quarry backend call fails for any reason — non-2xx, network
...
async def _raise_for_response(response: httpx.Response, *, context: str) -> None:
    """
    Convert an HTTP error response into :class:`QuarryClientError` with a
    ...
    async def _request(
        ...
        *,
        ...
        try:
            ...
            raise QuarryClientError(f"{context} failed: {exc.__class__.__name__}", status_code=None) from exc
...
class QuarryApiClient(_BaseClient):
    """
    ...
    def from_settings(
        ...
        *,
        ...
        return cls(
            base_url=s.QUARRY_API_BASE_URL,
            token=s.QUARRY_API_SERVICE_TOKEN,
            tenant_id=s.QUARRY_DEFAULT_TENANT_ID,
            timeout_seconds=s.QUARRY_HTTP_TIMEOUT_SECONDS,
            transport=transport,
        )
    ...
    async def list_open_cases(
        ...
        *,
        ...
        try:
            ...
            raise QuarryClientError("list cases failed: invalid JSON") from exc
        if not isinstance(rows, list):
            raise QuarryClientError("list cases failed: unexpected response shape")
    ...
    async def get_case(self, case_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise QuarryClientError("get case failed: invalid JSON") from exc
    ...
    async def launch_investigation(
        ...
        *,
        ...
        try:
            ...
            raise QuarryClientError("launch investigation failed: invalid JSON") from exc

    async def get_case_summary(self, case_id: str) -> dict[str, Any]:
        """
        Fetch the structured per-case auto-summary used by ``/quarry explain``.
        """
        safe_id = quote(case_id, safe="")
        response = await self._request(
            "GET",
        ...
        try:
            ...
            raise QuarryClientError("case summary failed: invalid JSON") from exc
...
class QuarryActionsClient(_BaseClient):
    """
    ...
    def from_settings(
        ...
        *,
        ...
        token = s.QUARRY_ACTIONS_SERVICE_TOKEN or s.QUARRY_API_SERVICE_TOKEN
        return cls(
            base_url=s.QUARRY_ACTIONS_BASE_URL,
            token=token,
            tenant_id=s.QUARRY_DEFAULT_TENANT_ID,
            timeout_seconds=s.QUARRY_HTTP_TIMEOUT_SECONDS,
            transport=transport,
    ...
    async def submit_action(
        ...
        *,
        ...
        try:
            ...
            raise QuarryClientError("submit action failed: invalid JSON") from exc
    ...
    async def approve_action(self, action_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise QuarryClientError("approve action failed: invalid JSON") from exc
    ...
    async def reject_action(self, action_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise QuarryClientError("reject action failed: invalid JSON") from exc
