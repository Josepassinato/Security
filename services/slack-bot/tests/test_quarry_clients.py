...
* QuarryClientError translation for non-2xx responses and transport errors
...
def _settings() -> SlackBotSettings:
    return SlackBotSettings(
        ...
        QUARRY_API_BASE_URL=API_BASE,
        QUARRY_ACTIONS_BASE_URL=ACTIONS_BASE,
        QUARRY_API_SERVICE_TOKEN=TOKEN,
        QUARRY_ACTIONS_SERVICE_TOKEN=TOKEN,
        QUARRY_DEFAULT_TENANT_ID=TENANT,
    )
...
async def test_list_open_cases_filters_closed_and_caps_to_limit():
    ...
    with respx.mock() as router:
        ...
        client = QuarryApiClient.from_settings(settings, transport=httpx.MockTransport(router.handler))
        try:
...
async def test_non_2xx_raises_aisoc_client_error_with_status():
    ...
    with respx.mock() as router:
        ...
        client = QuarryApiClient.from_settings(settings, transport=httpx.MockTransport(router.handler))
        try:
            with pytest.raises(QuarryClientError) as exc:
...
async def test_invalid_json_raises_client_error():
    ...
    with respx.mock() as router:
        ...
        client = QuarryApiClient.from_settings(settings, transport=httpx.MockTransport(router.handler))
        try:
            with pytest.raises(QuarryClientError):
...
async def test_submit_action_posts_full_payload_with_tenant():
    ...
    with respx.mock() as router:
        ...
        client = QuarryActionsClient.from_settings(settings, transport=httpx.MockTransport(router.handler))
        try:
...
async def test_actions_client_falls_back_to_api_token_when_actions_token_missing():
    settings = SlackBotSettings(
        QUARRY_API_BASE_URL=API_BASE,
        QUARRY_ACTIONS_BASE_URL=ACTIONS_BASE,
        QUARRY_API_SERVICE_TOKEN="aisoc_api_only",
