"""Async client for the Quarry built-in osquery TLS endpoint.
...
way as any other Quarry credential.
"""
...
class QuarryDirectError(RuntimeError):
    """Raised when an quarry-direct API call fails."""


class QuarryDirectClient:
    """Async client for the Quarry built-in osquery TLS service.
    ...
    async def _enqueue_for_hosts(
        ...
        for host in target_hosts:
            try:
                ...
                raise QuarryDirectError(f"Failed to enqueue query for host {host}: {exc}") from exc
        return query_ids
